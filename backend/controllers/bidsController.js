...
module.exports.createBid = catchAsync(async (request, response, next) => {
       ...
       const {nickname, bid, virtualCredits, openingBid, username, reputationPoints, creditsLeft} = request.body;

      
        const newBid = await Bid.create({nickname, bid, virtualCredits, openingBid, username, reputationPoints, creditsLeft});
